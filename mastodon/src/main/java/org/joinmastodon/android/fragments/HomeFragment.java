...
public class HomeFragment extends AppKitFragment implements AssistContentProviderFragment {
	...
	// Registered while any non-feed space is active so back press returns to feed.
	...
	public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
		...
		// Fragment container — fills all space (behind the sheet)
		...
		// Bottom sheet container (hosts KronkHubFragment / KosmosSheet)
		...
		// Bottom nav bar — inflate into root
		...
		if (savedInstanceState == null) {
			// hubFragment goes in the sheet container
			getChildFragmentManager().beginTransaction()
					.add(sheetContainer.getId(), hubFragment)
					.commit();

			// Space fragments go in the main fragment container
			getChildFragmentManager().beginTransaction()
	...
	// Called by KronkHubFragment (via KosmosSheet) when a space planet is tapped
	public void openSpace(Space space) {
		// Collapse sheet when navigating to a space
	...
	private void switchToSpace(Space space) {
		...
		// Feed is the root — only add back callback when on a non-feed space
		if (space != Space.FEED) {
	...
	private android.app.Fragment fragmentForSpace(Space space) {
		switch (space) {
			...
			default: return feedFragment;  // FEED, NUDGES (no fragment yet), and fallback
		}
	}
	...
	private void onHubTapped() {
		if (showingNotifications) {
			// Exit notifications overlay back to feed
			showingNotifications = false;
			getChildFragmentManager().beginTransaction()
					.hide(notificationsFragment)
					.show(feedFragment)
		...
		// Cycle sheet: collapsed -> half-expanded -> expanded -> collapsed
