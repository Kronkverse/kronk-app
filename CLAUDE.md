...
- **Deep links:** `kronk-auth://callback` for OAuth, `https://kronk.info/@*` and `/invite/*` for app links (the production host is moving from `mastodon.kronk.info` to `kronk.info` as part of the 2.0 rebrand — the intent-filter in `AndroidManifest.xml` needs to cover both during the transition)
